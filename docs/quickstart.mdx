...
The `useAssistant()` hook from modifywithai returns an object that is directly compatible with CommandMenu's `chat` prop:
...
The `agenticActions` serve double duty — they appear as searchable commands in command mode, and the modifywithai agent can invoke them in chat mode (with user approval).
